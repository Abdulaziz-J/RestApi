                    ...
                    @foreach ($comments as $comment)
                        <li class="mt-2 mb-2">
                            ...
                            @if($comment->iamge_name)
                                <img src="/images/{{$comment->iamge_name}}" alt="" style="width:250px"
                                     alt="comment-image" class="img mb-2 mt-2">
                            @endif
                            <br>
                            <strong>By {{$comment->user->name}}</strong> : {{$comment->content}}

                            <div class="container mb-4 mt-4">
                                <button class="like" data-id="{{$comment->id}}" role="button" href="#">
                                    <i class="fa fa-thumbs-o-up"></i> Like
                                    <span>{{ $comment->likes == null ? 0: $comment->likes->likes}}</span>
                                </button>
                                <button class="dislike" data-id="{{$comment->id}}" role="button" href="#">
                                    <i class="fa fa-thumbs-o-down"></i> Dislike
                                    <span>{{ $comment->likes == null ? 0: $comment->likes->dislikes}}</span>
                                </button>
                            </div>
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form_main">
                        ...
                        <div class="form">
                            <form action="/comments/create/{{$lesson->id}}" method="post" enctype="multipart/form-data"
                                  id="comment_form" name="contactFrm">

                                <input type="hidden" value="{{csrf_token()}}" id="token" name="_token">

                                <textarea id="content" placeholder="Your Message" name="content" type="text"
                    ...
                    <script>
                        ...
                        $(document).ready(function () {
                            ...
                            $('.like').on('click', function () {
                                ...
                                console.log('Liked on ', comment_id);
                                var comment = $(this).find('span');
                                $.ajax({
                                    method: 'POST',
                                    url: '/like',
                                    data: {'comment_id': comment_id, '_token': _token, 'like': 1},
                                    success: function (d) {
                                        console.log(d.likes);
                                        comment.text(d.likes);
                                    }
                                });
                            });
                            $('.dislike').on('click', function () {
                                ...
                                $.ajax({
                                    method: 'POST',
                                    url: '/like',
                                    data: {'comment_id': comment_id, '_token': _token, 'dislike': 1},
                                    success: function (d) {
                                        console.log(d.likes);
                                        comment.text(d.dislikes);
                                    }
                                });
                            });
